...
DATASET_FILE_CANDIDATES = {
    "mMTC": ["mMTC.csv", "mMTC.arff"],
    "URLLC": ["URLLC.csv", "URLLC.arff"],
    "eMBB": ["eMBB.csv", "eMBB.arff"],
    "TON_IoT": ["train_test_network.csv"],
    "train_test_network": ["train_test_network.csv"],
}
...
def list_dataset_files():
    ...
    for k in ["mMTC", "URLLC", "eMBB", "TON_IoT"]:
        try:
            out[k] = str(resolve_dataset_path(k))
...
def resolve_dataset_path(dataset_name: str) -> Path:
    ...
    candidates = DATASET_FILE_CANDIDATES.get(
        dataset_name, [DATASET_FILES[dataset_name]]
    )
    ...
    raise FileNotFoundError(
        f"File not found for {dataset_name}: {', '.join(candidates)}"
    )


def _decode_arff_strings(df: pd.DataFrame) -> pd.DataFrame:
...
def _read_dataset_file(path: Path) -> pd.DataFrame:
    if path.suffix.lower() == ".arff":
        ...
        return _decode_arff_strings(df)

    with open(path, "r", encoding="utf-8", errors="replace") as f:
        first_line = f.readline()

    sep = ";" if first_line.count(";") > first_line.count(",") else ","
    print(f"{path.name}: detected separator={repr(sep)}")
    return pd.read_csv(path, sep=sep, low_memory=False, keep_default_na=False)


def load_dataset(dataset_name: str):
    dataset_name = normalize_dataset_name(dataset_name)
    filename = resolve_dataset_path(dataset_name)
    df = _read_dataset_file(filename)
    ...
    print(f"Loaded {dataset_name} from {filename.name}: {df.shape}")
...
def prepare_data(dataset_name=None):
    ...
    if dataset_name == "TON_IoT" and len(df) > 50000:
        df = df.groupby("Label", group_keys=False).apply(
            lambda frame: frame.sample(
                n=max(1, round(len(frame) * 50000 / len(df))),
                random_state=42,
            )
        )
        print(f"TON_IoT -> sampled dataset for training: {df.shape}")
...
def train_model(
    ...
    if validate_dataset is not None:
        validate_dataset(dataset_name, info["df"])
        if log_callback is not None:
            log_callback(f"Dataset validation passed for {dataset_name}")
    ...
    mlflow_enabled = not mlflow_managed
    mlflow_run = None
    ...
    try:
        ...
        if log_callback is not None:
            log_callback(f"Accuracy: {acc:.4f}")
            log_callback(f"F1: {f1:.4f}")
            log_callback(f"Precision: {precision:.4f}")
            log_callback(f"Recall: {recall:.4f}")
            log_callback(f"AUC: {auc:.4f}")
        ...
        destination = (
            Path(artifact_path) if artifact_path else model_path_for(dataset_name)
        )
        joblib.dump(bundle, destination)
        print(f"[OK] Model saved: {destination}")
        if log_callback is not None:
            log_callback(f"Model artifact saved to {destination}")
